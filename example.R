...
# --- Generate data ---
dat <- simdata(n=1000, gamma=1.5, rho=-1.5, psi11=0.5, psi21=-2, alpha0=-1.098612, cens=2.18)
...
# Be sure to check dataset has removed any values with NA for exposure, covariates, event time, and event type
covariates <- dat[, c('z1', 'z2', 'z3', 'z4', 'z5')]
...
ipcw_res <- rmtl_ipcw(times=dat$x, event=dat$event, eoi=1, group=dat$a, tau=tau, cov=as.matrix(covariates), strata=FALSE)




# --- Plot IPW estimator with and without accounting for competing risks (1- Kaplan Meier method, Conner Stat Med 2019)
...
# Notice beta for 'group' is larger than estimate accounting for compting risk 

# This result is equivalent to using rmst2() in package survRM2
ipcw_res_ignore <- rmtl_ipcw(times=dat$x, event=dat$i.event1, eoi=1, group=dat$a, tau=tau, cov=as.matrix(covariates), strata=FALSE)
