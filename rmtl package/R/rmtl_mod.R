### Function for RMTL regression model for competing risks data estimated with IPCW
...
#' Regression model for the restricted mean time lost (RMTL) difference estimated wtih inverse probability of censoring (IPCW)
#'
...
#' @param tau time horizon used to restrict event times
#' @param cov matrix of covariates
#' @param strata optional argument to stratify censoring weights in subgroups (as desginated in group)
#' @param group  optional factor to stratify censoring weights in subgroups. Should only be used when strata=TRUE
#'
#' @return estimated regression model for the difference in cause-specific RMTL
#' @export
#'
#' @examples
rmtl_mod <- function(times, event, eoi=1, tau, cov=NULL, strata=FALSE, group=NULL){
  ...
  if(strata==TRUE){
    for(aa in 1:length(unique(group))){
      ...
      # Derive IPCW
      fit <- survival::survfit(Surv(y.a, d0.a) ~ 1)
  ...
  lm.fit <- stats::lm(delta1*(tau-y) ~ x-1, weights=w)
  ...
  res <- cbind(beta=lm.fit$coef, se=se, cil=lm.fit$coef-(1.96*se), ciu=lm.fit$coef+(1.96*se),
               z=lm.fit$coef/se, p=2*(1-stats::pnorm(abs(lm.fit$coef/se))))
